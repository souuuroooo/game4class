...
class GameScene_test:
    ...
    #p1 hit box
        ...
        self.box_shape_left.elasticity = 1
        ...
        self.box_shape_right.elasticity = 1
    ...
    #p2 hit box  
        ...
        self.box_at_shape.elasticity = 1
        ...
        self.box_at_shape_left.elasticity = 1
        ...
        self.box_at_shape_right.elasticity = 1
    ...
    #p2 hit box end
        ...
        #素材
        self.word = pygame.image.load(os.path.join("material", "A.png")).convert_alpha()       
        broad = pygame.image.load(os.path.join("material", "broad.png")).convert_alpha()
        self.broad = pygame.transform.scale(broad,(300,300))
        self.b_count=0
        net = pygame.image.load(os.path.join("material", "net.png")).convert_alpha()
        self.net=pygame.transform.scale(net,(self.widea+10,self.highta+10))
        neko = pygame.image.load(os.path.join("material", "nekoo.png")).convert_alpha()
        nekoo = pygame.transform.flip(neko,1,0)
        self.neko1=pygame.transform.scale(nekoo,(300,250))
        neko = pygame.image.load(os.path.join("material", "nekooo.png")).convert_alpha()
        nekoo = pygame.transform.flip(neko,1,0)
        ...
        self.wuii = []
        for i in range(25):
            img = pygame.image.load(os.path.join("wuiiai", f"{i}.png")).convert_alpha()
    ...
    def update(self, events ,screen):       
        for event in events:
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_d:
                    self.ball_body.velocity = (250, self.ball_body.velocity.y)
                    self.box_body_right.velocity = (250, self.box_body_right.velocity.y)
                    self.timer = 120

                if event.key == pygame.K_a:
                    self.ball_body.velocity = (-250, self.ball_body.velocity.y)
                    self.box_body_left.velocity = (-250, self.box_body_left.velocity.y)
                    self.timer = -120
                #p1 move end
                #p2 move
        ...
        #啟動技能
        if g_var.back_from_skill:   # 這個變數你可以在 skill.py 結束前設 True
    ...
    def draw(self, screen):
        ...
        # 判斷用不用鼻孔的動畫 #draw ball 1      
        ball_pos = self.pymunk_to_pygame(self.ball_body.position)
        if g_var.get_char == "pika":
            self.pika.draw(screen, ball_pos, -10)
        elif g_var.get_char == "gob":
            self.gob.draw(screen, ball_pos, -10)
        elif g_var.get_char == "cat":
            self.cat.draw(screen, ball_pos, -25)
        else:
            ball_pos = self.pymunk_to_pygame(self.ball_body.position)
        ...
        if g_var.get_char_2 == "pika":
            self.pika.draw(screen, ball_at_pos, -10)
        elif g_var.get_char_2 == "gob":
            self.gob.draw(screen, ball_at_pos, -10)
        elif g_var.get_char_2 == "cat":
            self.cat.draw(screen, ball_at_pos, -25)
        else:
       ...
       # 球             
        ...
        text1 = self.lable1.render(f"{g_var.R_point}", True, "red")
        text2 = self.lable1.render("t:加左邊 y:加右邊", True, "red")
